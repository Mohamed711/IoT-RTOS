...
void spiInit(void)
{
	...
	{
		if(spi_clk[u8LoopCount].u32TempFreq <= SPI_InitConfig.u32MaxFreq)
		{
			break;
		}
	}
	...
	SPCR_REG = 0X00;
	SPCR_REG |=  SPI_EN | SPI_InitConfig.u8DataOrder |Master_Or_Slave| SPI_InitConfig.u8Mode|(spi_clk[u8LoopCount].u8RegVal & 0x03);
	SPSR_REG = 0x00;
